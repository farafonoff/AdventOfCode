...
function createArray(length) {
    var arr = new Array(length || 0).fill(false),
        i = length;
...
var re = /(turn on|turn off|toggle) ([0-9]+),([0-9]+) through ([0-9]+),([0-9]+)/
...
function count() {
  return lights.reduce((s,e)=>s+e.reduce((s,ei)=>ei?s+1:s,0),0)
}

var input = long_input()

input.split('/n').forEach(s=>exec(parse(s)))
count(lights)
